...
def _get_litellm_cost_map(model, completion_window="*", provider="default"):
    cost = external_model_cost(
        model, completion_window=completion_window, provider=provider
    )
    litellm_cost_map = {
        model: {
            "max_tokens": 8192,
...
def external_model_cost(model, completion_window="*", provider="default"):
    ...
    cost = _DEFAULT_COST_MAP[provider]["cost"][model]["input_cost_per_million"][
        completion_window
    ]
...
class _KlusterAICostProcessor(_LitellmCostProcessor):
    ...
    def cost(self, *args, completion_window="*", **kwargs):
        ...
        if (
            _KlusterAICostProcessor._wrap(model, completion_window)
            in _KlusterAICostProcessor._registered_models
        ):  #
        ...
        litellm.register_model(
            _get_litellm_cost_map(
                model, provider="external", completion_window=completion_window
            )
        )
        _KlusterAICostProcessor._registered_models.add(
            _KlusterAICostProcessor._wrap(model, completion_window)
        )
